...
export const addRoomLabelDivs = (divs: any, eyeIcon: p5Types.Image, font: p5Types.Font, p5: p5Types) => {
    ...
    for (let i = 0; i < rooms.length; i++) {
        let roomL = new RoomLabel(p5, i, eyeIcon, font);
...
export const addColumnDivs = (divs: any, columnGif: p5Types.Image, p5: p5Types, factor = 1) => {
    ...
    for (let i = 0; i < numCols; i++) {
        let column = new ShadowDraggable(i, gx + dx * i, gy - dy * i, 80 * factor, 280 * factor, p5, columnGif)
        // columns.push(column)
        // column.initMask();
    ...
    let x = 1.5 * sc;
    let y = 22.5 * sc;
    divs.columns.push(new ShadowDraggable(numCols, x, y, 80 * factor, 280 * factor, p5, columnGif))
    divs.columns.push(new ShadowDraggable(numCols + 1, x + sc * 2, y, 80 * factor, 280 * factor, p5, columnGif))

    x = -3.5 * sc;
    y = 17.5 * sc;
    divs.columns.push(new ShadowDraggable(numCols + 2, x, y, 80 * factor, 280 * factor, p5, columnGif))
    divs.columns.push(new ShadowDraggable(numCols + 3, x + sc * 2, y, 80 * factor, 280 * factor, p5, columnGif))
}
