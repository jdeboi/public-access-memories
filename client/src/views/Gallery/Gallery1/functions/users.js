...
export function drawUser(user, p5, imgs) {
    ...
    if (user.isSpeaking) {
        p5.push();
        p5.translate(-17, 0);
        if (user.isMuted || user.isGlobalMuted)
            drawMicOff(p5, imgs[5]);
        else
            drawMic(p5, imgs[5]);
        p5.pop();
    }
...
function drawChat(p5, chat) {
    // p5.text("💬", 22, -14);
    if (chat)
        p5.image(chat, 22, -54, iconSize, iconSize);
}

function drawCocktail(p5, cocktail) {
    // p5.text("🍸", 20, 20);
    if (cocktail)
        p5.image(cocktail, 20, 0, iconSize, iconSize);
}
...
function drawWine(p5, wine) {
    // p5.text("🍷", -20, 20);
    if (wine)
        p5.image(wine, -20, 0, iconSize, iconSize);
}

function drawMic(p5, mic) {
    if (mic)
        p5.image(mic, -5, 10, iconSize / 1.5, iconSize / 1.5)
}

function drawMicOff(p5, mic) {
    drawMic(p5, mic);
    p5.stroke("red");
    p5.strokeWeight(5);
    p5.line(0, 0, 10, 10);
