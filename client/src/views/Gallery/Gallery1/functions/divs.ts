...
export const displayRoomLabelDivs = (font: p5Types.Font, roomCount: any, divs: any) => {
...
export const addDoorDivs = (divs: any, doors: any, doorImgs: p5Types.Image[], p5: p5Types) => {
    divs.doors = [];
    let numDoors = 4;
...
export const addColumnDivs = (divs: any, columnGif: p5Types.Image, p5: p5Types, factor = 1) => {
    ...
    let x = 1.5 * sc;
    let y = 22.5 * sc;
    divs.columns.push(new ShadowDraggable(numCols, x, y, 80 * factor, 280 * factor, p5, columnGif, GlobalConfig))
    divs.columns.push(new ShadowDraggable(numCols + 1, x + sc * 2, y, 80 * factor, 280 * factor, p5, columnGif, GlobalConfig))

    x = -3.5 * sc;
    y = 17.5 * sc;
    divs.columns.push(new ShadowDraggable(numCols + 2, x, y, 80 * factor, 280 * factor, p5, columnGif, GlobalConfig))
    divs.columns.push(new ShadowDraggable(numCols + 3, x + sc * 2, y, 80 * factor, 280 * factor, p5, columnGif, GlobalConfig))
}
...
export const addOakDivs = (divs: any, oakImg: p5Types.Image, p5: p5Types) => {
    ...
    for (let i = 0; i < 1; i++) {
        let oak = new Draggable(i, sc * -7.5, sc * 34, 500, 400, p5, oakImg, GlobalConfig);
        divs.oaks.push(oak);
    }
}

export const addSwingDivs = (divs: any, baby: p5Types.Image, chain: p5Types.Image | null, p5: p5Types) => {
    divs.swings = [];
    divs.swings.push(new Swing(0, -4.1, 36.8, 100, 190, p5, baby, chain, GlobalConfig));
}

export const addTableDivs = (divs: any, tableImgs: p5Types.Image[], p5: p5Types) => {
    let sc = GlobalConfig.scaler;
    divs.tables = [];
    let tables = [
        { x: -9, y: -2.25 },
    ...
    for (let i = 0; i < 3; i++) {
        let { x, y } = tables[i];
        let table = new Table(i, x, y, sc * 3, sc * 3, p5, tableImgs[0], tableImgs[1], GlobalConfig);
...
export const addBarDivs = (divs: any, lightImg: p5Types.Image, p5: p5Types) => {
    ...
    for (const barType of barTypes) {
        ...
        switch (barType) {
            case "wine":
                divs.bars.push(new WineBar(i, { ...bar }, lightImg, numBarItems, p5, GlobalConfig));
                break;
            case "cocktail":
                divs.bars.push(new CocktailBar(i, { ...bar }, lightImg, numBarItems, p5, GlobalConfig));
                break;
            case "DJ":
                divs.bars.push(new DJBar(i, { ...bar }, lightImg, numBarItems, p5, GlobalConfig));
                break;
            case "cheese":
                divs.bars.push(new CheeseBar(i, { ...bar }, lightImg, numBarItems, p5, GlobalConfig));
                break;
        }
        i++;
    }
...
export const displayDoorDivs = (userX: number, userY: number, divs: any, isClosed: boolean, closedSign: p5Types.Image) => {
...
export const displayTreeDivs = (userX: number, userY: number, sz: number, divs: any) => {
...
export function updateDivs(userEase: { x: number, y: number }, users: any, divs: any, isPanGallery = false) {
...
export const checkFolderDivsDouble = (userX: number, userY: number, divs: any) => {
    for (const folder of divs.folders) {
        folder.checkDoubleClicked(userX, userY);
    }
}

export const checkTrashDivsDouble = (userX: number, userY: number, divs: any) => {
    for (const trash of divs.trashCans) {
