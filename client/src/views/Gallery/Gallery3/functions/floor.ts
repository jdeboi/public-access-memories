...
import { rooms, roomConfig } from '../../../../data/FieldsOfView/RoomConfig';
...
export const drawTraces = (p5: p5Types) => {
    ...
    const lines = [
        ...
        [
            ...
            [getDoorX(rooms[0])+180+380, getDoorY(rooms[0])+230+105+100+320],
            [getDoorX(rooms[0])+180+680, getDoorY(rooms[0])+230+105+100+320],
        ],
    ]
...
export const drawSpaceFloorLine = (x0: number, y0: number, x1: number, y1: number, w: number, p5: p5Types) => {
...
const drawPatchDouble = (x0:number, y0:number, w:number, h: number, rot:number, p5:p5Types) => {
    drawGoPatch(x0, y0, w, h, rot, p5);
    drawGoPatch(x0+w, y0+h+20, w, h, rot+Math.PI, p5);
}

const drawGoPatch = (x0:number, y0:number, w:number, h: number, rot:number, p5:p5Types) => {
    p5.push();
    p5.translate(x0, y0);
    p5.rotate(rot);
    p5.strokeWeight(5);
    ...
    for (let i = 0; i < num; i++) {
        let maxA = 150;
        let alpha = maxA/2*p5.sin(p5.millis()/200+i/10*2*p5.PI) + maxA/2;
        // if (dir == -1) 
        //     alpha = 255 - alpha;
