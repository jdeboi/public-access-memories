...
import { artists, rooms, roomConfig } from '../../../../data/FieldsOfView/RoomConfig';
...
class FOVRoom extends Room {
    ...
    displayAnaglyph(img: p5Types.Graphics) {
        ...
        displayWall({ x: dx, y: dy + h }, { x: dx + w * .3, y: dy + h }, this.p5);
        displayWall({ x: dx + w * .7, y: dy + h }, { x: dx + w, y: dy + h }, this.p5);
    ...
    roomBoundaryCrossing(prevStep: { x: number, y: number }, userStep: { x: number, y: number }) {
        // TODO
    ...
    roomDoorCrossing(prevStep: { x: number, y: number }, userStep: { x: number, y: number }, id = 0) {
