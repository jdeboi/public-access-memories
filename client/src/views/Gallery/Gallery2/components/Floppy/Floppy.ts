...
export default class Floppy extends Draggable {
    ...
    constructor(p5: p5Types, id: number, x: number, y: number, link: string, eyeIcon: p5Types.Image, img: p5Types.Image, slider: p5Types.Image, font: p5Types.Font, connection: { x: number, y: number }) {
        ...
        const buttonW = this.w *.5;
        this.mobileButton = new FloppyButton((this.w-buttonW) / 2, -buttonW*.5, buttonW, .4*buttonW, p5);

        this.dragCount = this.p5.frameCount;
    }

    getTrace() {
        let loopX = this.x + this.w*.5; //+ this.w *.8;
        let loopY = this.y + this.h + 30;

        const len = GlobalConfig.scaler * 2;
        return { startX: loopX, startY: loopY + 15, endX: loopX, endY: loopY + len }
    }
    ...
    displayContent(userX: number, userY: number) {
        ...
        this.p5.image(this.slider, -this.openAmt + 22, 0, this.slider.width * fac, this.slider.height * fac);
    }
    ...
    displayBars() {
        ...
        for (let i = 0; i < numBars; i++) {
            ...
            const col = this.p5.lerpColor(c1, c2, this.p5.map(i, 0, numBars-1, 0, 1))
            this.p5.fill(col);
    ...
    displayLabel(roomCount: any) {
        ...
        let x = roomConfig.w * GlobalConfig.scaler * .2;

        this.label.displayFloppy(x, this.h * .65, roomCount);
        this.p5.pop();
    }
