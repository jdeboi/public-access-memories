...
export default class HardDrive extends ShadowDraggable {
  ...
  constructor(
    ...
    this.drive = p5.loadImage(
      "https://jdeboi-public.s3.us-east-2.amazonaws.com/public_access_memories/as_i_recall/gallery/drive.png"
    );
    this.needle = p5.loadImage(
      "https://jdeboi-public.s3.us-east-2.amazonaws.com/public_access_memories/as_i_recall/gallery/needle.png"
    );
  }
  ...
  displayContent() {
    ...
    this.p5.rotate(0.06 + 0.2 * this.p5.sin(this.p5.millis() / 200));
    const wn = this.needle.width * factor;
    const hn = this.needle.height * factor;
    this.p5.image(this.needle, -wn * 0.3, -hn * 0.8, wn, hn);
    this.p5.pop();
