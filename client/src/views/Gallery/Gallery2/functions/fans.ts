...
export const addFanDivs = (fans: FanDraggable[], p5: p5Types) => {
    ...
    for (let i = 0; i < fanLocs.length; i++) {
        fans.push(new FanDraggable(i, fanLocs[i][0]*scaler,fanLocs[i][1]*scaler, 4*scaler, 4*scaler, p5))
    }
}

export const checkFanDivs = (x: number, y: number, fans: FanDraggable[], hardDrive: HardDrive) => {
...
export const displayFans = (x: number, y: number, fans: FanDraggable[], hardDrive: HardDrive) => {
    for (const fan of fans) {
        fan.display(x, y);
