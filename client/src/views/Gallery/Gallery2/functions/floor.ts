...
const drawSpaceFloor = (x0: number, y0: number, w: number, h: number, div: number, p5: p5Types, isDark = false) => {
...
const drawTraces = (floppies: Floppy[], p5: p5Types) => {
    const lines = [
        [
            ...
            [floppies[2].getTrace().endX + 4 * GlobalConfig.scaler, floppies[2].getTrace().endY],
            [floppies[2].getTrace().endX + 4 * GlobalConfig.scaler, floppies[0].getTrace().endY]
        ],
        [
            ...
            [15 * GlobalConfig.scaler, (floppies[6].getTrace().endY + floppies[7].getTrace().endY) / 2],
...
export const setTraceStroke = (p5: p5Types, alpha = 150, sw=10, col='#4bdb88') => {
...
const drawFloor = (x0: number, y0: number, w: number, h: number, isDark: boolean, isFilled: boolean, spacing: number, p5: p5Types) => {
    // let spacing = GlobalConfig.scaler;
    let d = new Date();
    ...
    for (let x = x0 * sc; x < (x0 + w) * sc; x += spacing) {
        for (let y = y0 * sc; y < (y0 + h) * sc; y += spacing) {
            ...
            if (isFilled) {
                ...
                let d = new Date();
                alpha = p5.map(Math.sin(d.getTime() / 1000 + x / 100 + y / 200), -1, 1, 0, 110);
                p5.fill(255, alpha);
            } else {
