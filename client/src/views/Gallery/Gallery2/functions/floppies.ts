...
import { artists, rooms } from "../../../../data/Shows/AsIRecall/RoomConfig";
...
export const addFloppyDivs = (
  floppies: Floppy[],
  eyeIcon: p5Types.Image,
  floppyImg: p5Types.Image,
  ...
  for (let i = 0; i < rooms.length; i++) {
    ...
    let connection = p5ToWorldCoords(
      connections[i][0],
      connections[i][1],
      GlobalConfig
    );
...
export const displayFloppyDivs = (
  x: number,
  y: number,
  roomCount: any,
  isMobile: boolean,
...
export const endFloppyDivDrag = (floppies: Floppy[], p5: p5Types) => {
  for (const floppy of floppies) {
    if (floppy.endFloppyDrag(p5)) return floppy.endFloppyDrag(p5);
  }
  return null;
};

export const checkFloppyDivsDouble = (
  userX: number,
  userY: number,
  floppies: Floppy[]
) => {
