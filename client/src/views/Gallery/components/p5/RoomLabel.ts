...
export default class RoomLabel extends Draggable {
    ...
    constructor(p5: p5Types, id: number, eyeIcon: p5Types.Image, font: p5Types.Font, GlobalConfig: any, artists: any, rooms: any, roomConfig: any) {
        super(id, 0, 0, 160, 80, p5, null, GlobalConfig);
        ...
        const point = p5ToWorldCoords(room.x, room.y, GlobalConfig);
    ...
    displayFloppyContent(count: number) {
        const factor = this.GlobalConfig.scaler/40;
        this.p5.push();


        // TODO - guesswork on max number of letters in label
        const w = this.roomConfig.w * this.GlobalConfig.scaler * .75;
