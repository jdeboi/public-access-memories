...
const Sup = ({ id }: { id: number }) => {
  return (
    <sup id={`sup-${id}`}>
      <a style={{ color: "blue" }} href={`#fn-${id}`} role="doc-noteref">
        [{id}]
      </a>
    </sup>
  );
...
export const BrizIntroData: {
  content: React.ReactNode;
  footnotes: React.ReactNode[];
} = {
  content: (
    <>
      <h2>How To Glitch AI</h2>
      <p className="text-right">&lt;Nick Briz.2023&gt;</p>
      <p>
        What follows are a series of perspectives, which i'm calling
        &lt;VECTORS&gt;, from which we might begin to consider how, as glitch
        artists, we might approach producing glitch art w/artificial neural
        networks: the algorithms beginning to define this latest era of AI +
        Machine Learning.
        <Sup id={1} /> Though I will be focusing primarily on images &&
        communicating through text, it’s important to remember that glitch art
        can be experienced in multiple dimensions. Glitch art is an
        interdisciplinary + multi-modal artistic practice, genre, tag, scene,
        community && aesthetic. Diff artists have diff ways of defining the
        practice as well as its central concept. My fav definition of a g̴̤̀l̵̹̓i̶̤̍t̴̫̕c̵̫̃ḥ̸̕
        is: an unexpected moment in a system that, by catching us off-guard,
        reveals aspects of that system that might otherwise have gone unnoticed.
        At its best, a glitch is an epiphany. My goal w/these &lt;VECTORS&gt; is
        to suggest ways we might set ourselves up for revelatory surprise. U
        might find some of the &lt;vector-views&gt; (the accompanying images)
        compelling, but it's in the process of creating glitch art that we
        develop a visceral understanding of the logic + politics mediating our
        digital lives, && it's this practice i'm aiming on expanding through
        these &lt;VECTORS&gt;. These are not prescriptions, there is no "right
        way" to make glitch art, there are only critical, experimental &&
        playful ways of engaging systems the "wrong way".
      </p>
    </>
  ),
  footnotes: [
    <div id="fn-1">
      ...
      artists during https://not.gli.tc/h in Chicago, as well as on our
      💻🤖🧠🦾👨🏻💻artificial-unintelligence thread on our discord server, which
      all are welcome to join
    </div>,
  ],
};

export const BrizInProgressData: BrizQuadType = {
  id: 4,
  vector: "<VECTOR 5>",
  label: "in-progress",
  imgUrl: "#",
  didactic: "In Progress",
  content: (
    <>
      <h2>In Progress</h2>
      <p>
        As artists, cultural producers or knowledge workers of any kind,
        ignoring the cultural implications of AI means risking irrelevance. It
        can be hard to keep up w/the fast pace of technological "innovation",
        but setting aside some time for a bit of "disnovation" is as effective
        (maybe better from this vector-view) as any other way of getting
        "there". I've suggested only a few &lt;VECTORS&gt; the specifics of
        which are less important than the general perspective ++ there are
        certainly many more &lt;VECTORS&gt; to be discovered by anyone willing
        to explore it actively. [i’m accepting pull requests]
      </p>
    </>
  ),
  footnotes: [
...
export const BrizData: BrizQuadType[] = [
  {
    ...
    content: (
      <>
        ...
        <p>
          ...
          text prompt "glitch art" to OpenAI’s DALL-E. i imagine many glitch
          artists would not consider this image a glitch in the purest sense,
          but rather a glitch-esque image or "glitch-alike”
          <Sup id={2} />: an image w/glitch aesthetics created by some other
          means. This image was not produced by corrupting the data, short
          circuiting the neural network or otherwise misusing an AI model, there
          is no misuse or malfunction occurring here, the model is operating
          exactly as intended.
        </p>
        <p>
          ...
          trained glitch artist's 👁️. These new AI models, however, create
          ...
          i'd get when corrupting video compression algorithms
          <Sup id={3} />
          , but when i looked closer i noticed peculiar artifacts,
          <Sup id={4} />
          not those associated with image/video compression, but rather
          artifacts unique to this AI image diffusion process. The macro blocks
          that appear when databending a JPEG should be uniform in size but
          these were not. The grid of pixels that become more apparent when
          glitching images should be perfectly aligned, but these were
          structured slightly diff'ly. In many of these "glitch art" prompted
          images the pixels almost looked as though they were painted by hand,
          the way Gerhard Richter would reproduce the artifacts of photography
          in his paintings.
          <Sup id={5} /> This is likely a reflection of the model's bias, i'd
          guess its training data contains less JPG glitches than it does hand
          drawn paintings.
        </p>
        <p>
    ...
    footnotes: [
      <div className="text-sm text-slate-600 mt-6" id="fn-2">
        [2] “glitch-alike” is a term coined by{" "}
        <a
      ...
      <div className="text-sm text-slate-600 mt-6" id="fn-3">
        [3] See my{" "}
        <a
  ...
  },
  {
    ...
    content: (
      <>
        <p>
          ...
          algorithmic logic behind each file format
          <Sup id={6} />. How might we reveal the algorithmic logic behind an
          AI, what perspective can we gain from this && what sorts of new
          glitchy artifacts await us? Using a piece of technology the "wrong"
          way has always been a core tenet of glitch art: we use music software
          to edit images, digital imaging tools to edit audio && text editors to
          edit everything but. Applying this logic to AI, we might try to create
          images w/a model that was trained to produce something else. This is
          precisely how the image in &lt;vector-view-2&gt; was created: using
          GoogLeNet
          <Sup id={7} />, an AI model not designed to create images, but instead
          trained to classify or "predict" the contents of an image.
        </p>
        <p>
        ...
        <p>
          ...
          in this case a collection of labeled images.
          <Sup id={8} /> At first the network gets it all wrong, but because the
          training data contains the right answer (the label or "class") we can
          adjust all its internal parameters w/the help of an otherwise fairly
          simple piece of calculus known as a loss function.
          <Sup id={9} /> Do that enough times (which often requires quite a bit
          of processing power) && the weights + biases ascend/descend to just
          the right values. They become abstract reflections of the patterns in
          the images associated w/each label. The trained model can almost
          perfectly classify all images, generalizing beyond the images it was
          trained on.
        </p>
        <p>
          ...
          confidence score for the “flamingo” class, in a sense, this image is
    ...
    footnotes: [
      ...
      <div className="text-sm text-slate-600 mt-6">
        ...
        was an deep neural network created by researchers at Google in 2014. It
        was trained on the{" "}
        <a
...

  {
    ...
    content: (
      <>
        ...
        <p>
          ...
          us what the most optimized settings for producing the “highest
          quality” image might be, like which sampling method to use (SDE is one
        ...
        <p>
          ...
          <Sup id={12} /> + modes of interfacing w/it. Most of the graphical
          apps created by the community (ex:{" "}
          <a
        ...
        <p>
          ...
          + other settings for achieving “high quality” results, ignoring these
    ...
    footnotes: [
      <div className="text-sm text-slate-600 mt-6">
        ...
        as well as experimenting w/Tim Sainburg’s{" "}
        <a
  ...
  },
  {
    ...
    content: (
      <>
        <p>
          ...
          speak?
          <Sup id={13} /> The better we understand these systems && the process
          occurring behind the scenes, the more likely we might conjure prompts
          capable of casting glitchy spells on the model. For example, we now
          know that diffusion models start by creating an image of colorful
          static known as "Gaussian noise", before de-noising it a number of
          times to produce an image which matches ur prompt. So what happens if
          we prompt it to make "Gaussian noise"?
          <Sup id={14} />
        </p>
        <p>
          Diffusion models are trained on an immense dataset
          <Sup id={15} /> of image/text-description pairs. These text
          descriptions are broken up into "tokens", which are often small words
          or word fragments ("graffiti", "pizza", "cat", "th", "ly", "ing" are
          all tokens found in Stable Diffusion's{" "}
          <a
    ...
    footnotes: [
      ...
      <div className="text-sm text-slate-600 mt-6">
        ...
        disappear as software “updates”. This is why glitch artists often avoid
        updating their software && even keep old computers/hardware on hand.
      </div>,
    ],
  },
