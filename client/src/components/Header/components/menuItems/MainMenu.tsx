...
import { IMainMenu, IListItem, IRoom } from "../../../../interfaces";
...
const MainMenu = (props: IMainMenu) => {
  ...
  const roomsList = rooms.map((rm: IRoom) => {
    let val: IListItem = {
      title: artists[rm.artistID].name,
      link: rm.link,
    };
    if (rm.shortcut) val.shortcut = rm.shortcut;
    if (rm.classN) val.classN = rm.classN;
    return val;
  });
  ...
  else if (isXXSmall()) return <React.Fragment></React.Fragment>;
  else return justTitle();
};
