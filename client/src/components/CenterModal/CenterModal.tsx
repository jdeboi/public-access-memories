...
export default function CenterModal({ title, classN, z, height, width, isHidden, content, buttons, isRelative, onHide }: CenterModalProps) {
    ...
    const [dimensions, setDimensions] = useState(getCalculatedDimensions(windowUI, isRelative, width, height));
    ...
    function getCalculatedDimensions(windowUI: any, isRelative: boolean, width?: number, height?: number) {
        ...
        if (height && width) {
            return {
                ...
                y: (windowUI.contentH - dim.h - windowUI.toolbarH) / 2 + (isRelative ? 0 : windowUI.headerH)
            };
        }
        return dim;
    }
                ...
                content={
                    ...
                    >
                        <div
                            ...
                            style={{ padding: 20, width: dimensions.w - 40, height: dimensions.h - 40 }}
                        >
                            {content}
                            {buttons}
                        </div>
